...
var allSteps=[], doneSet=new Set();

function saveDone(){var ids=[];doneSet.forEach(function(i){if(allSteps[i]&&allSteps[i].id)ids.push(allSteps[i].id);});try{localStorage.setItem('module_progress_'+MODULE_PK,JSON.stringify(ids));}catch(e){}}
function loadDone(){try{var r=localStorage.getItem('module_progress_'+MODULE_PK);if(!r)return;JSON.parse(r).forEach(function(id){var i=allSteps.findIndex(function(s){return s.id===id;});if(i!==-1)doneSet.add(i);});}catch(e){}}
function isStepDone(idx){return doneSet.has(idx);}
function markStepDone(idx){doneSet.add(idx);saveDone();var s=allSteps[idx];if(s&&s.id)fetch('/api/progress/step/'+s.id+'/complete/',{method:'POST',headers:{'Content-Type':'application/json','X-CSRFToken':getCSRF()}}).catch(function(){});renderPage();}

function checkQuizReturn(){var raw=localStorage.getItem('quizReturn');if(!raw)return;try{var r=JSON.parse(raw);if(String(r.moduleId)===String(MODULE_PK)){var idx=allSteps.findIndex(function(s){return s.id===r.stepId;});if(idx!==-1){doneSet.add(idx);saveDone();}try{var sc=JSON.parse(localStorage.getItem('module_scores_'+MODULE_PK)||'{}');sc[r.stepId]={score:r.score,correct:r.correctCount,total:r.totalCount};localStorage.setItem('module_scores_'+MODULE_PK,JSON.stringify(sc));}catch(e2){}}}catch(e){}localStorage.removeItem('quizReturn');}

/* ══════ Группировка ══════
   mainSteps: всё кроме practice, upload, final_exam (включая quiz!)
   practiceSteps: practice + upload
   finalSteps: final_exam
*/
...
/* ══════ Статусы с проверкой pass_score для quiz ══════ */
function isStepPassed(step){
  if(!isStepDone(step.idx)) return false;
  if(step.type==='quiz'){
    try{var sc=JSON.parse(localStorage.getItem('module_scores_'+MODULE_PK)||'{}');var d=sc[step.id];if(!d)return true;var ps=step.pass_score||70;return d.score>=ps;}catch(e){}
  }
  return true;
}

function getMainStepStatus(step){
  if(isStepDone(step.idx)){
    // quiz не сдан?
    if(step.type==='quiz'){
      try{var sc=JSON.parse(localStorage.getItem('module_scores_'+MODULE_PK)||'{}');var d=sc[step.id];if(d){var ps=step.pass_score||70;if(d.score<ps)return 'failed';}}catch(e){}
    }
    return 'done';
  }
  var mains=getMainSteps();
  for(var k=0;k<mains.length;k++){
    if(mains[k].idx>=step.idx)break;
    if(!isStepPassed(mains[k]))return 'locked';
  }
  return 'current';
}
...
function getFinalStepStatus(step){
  ...
  if(getMainSteps().every(function(s){return isStepPassed(s);})&&(getPracticeSteps().length===0||getPracticeSteps().every(function(s){return isStepDone(s.idx);})))return 'current';
  return 'locked';
}

function getScoreText(stepId){try{var sc=JSON.parse(localStorage.getItem('module_scores_'+MODULE_PK)||'{}');var q=sc[stepId];if(q)return' \u00b7 <span style="color:'+(q.score>=70?'var(--status-green)':'var(--status-red)')+';font-weight:700">'+q.score+'%</span> ('+q.correct+' \u0438\u0437 '+q.total+')';}catch(e){}return '';}
...
function finishCourse(){document.getElementById('modalText').innerHTML='\u0412\u044b \u0443\u0441\u043f\u0435\u0448\u043d\u043e \u0437\u0430\u0432\u0435\u0440\u0448\u0438\u043b\u0438 \u043c\u043e\u0434\u0443\u043b\u044c<br><strong>\u00ab'+MODULE_TITLE+'\u00bb</strong>.<br>\u041f\u043e\u0437\u0434\u0440\u0430\u0432\u043b\u044f\u0435\u043c!';document.getElementById('finishModal').classList.add('visible');}

function buildStepCard(step,cc,tl,sub,act,ext,num){
  var mt=TYPE_MAP[step.type]||'online';
  return '<div class="step-card '+cc+'" id="step-'+step.idx+'"><div class="step-card-inner"><div class="step-top-row"><div class="step-type-badge '+mt+'">'+(ICO[mt]||'')+' '+tl+'</div><div class="step-author-num"><span class="step-num">\u0428\u0430\u0433 '+num+'</span></div></div><div class="step-title">'+step.title+'</div><div class="step-bottom-row">'+sub+'<div class="step-actions">'+act+'</div></div></div>'+ext+'</div>';
}
...
function renderPage(){
  ...
  var allMainDone=mainSteps.every(function(s){return isStepPassed(s);});
  ...
  var stepNum=0;
  /* main steps: material, quiz, slide — в порядке order, без разделителей */
  var mainHtml=mainSteps.map(function(step){
    ...
    if(st==='failed'){
      // Quiz не сдан
      cls='practice-pending';
      var fd;try{var sc=JSON.parse(localStorage.getItem('module_scores_'+MODULE_PK)||'{}');fd=sc[step.id];}catch(e){}
      var fps=step.pass_score||70;
      sub='<div class="step-subtitle" style="color:var(--status-red);">\u2717 \u041d\u0435 \u0441\u0434\u0430\u043d\u043e: '+(fd?fd.score:0)+'% (\u043d\u0443\u0436\u043d\u043e '+fps+'%)</div>';
      act='<button class="btn-step open" onclick="openStep('+step.idx+')">'+ICO_OPEN+' \u041f\u0435\u0440\u0435\u0441\u0434\u0430\u0442\u044c</button>';
    } else if(st==='done'){
      cls='done';
      var dt='\u0417\u0430\u0432\u0435\u0440\u0448\u0435\u043d\u043e';
      ...
      if(step.type!=='quiz'){ext='<div class="step-confirm" id="confirm-'+step.idx+'"><div class="step-confirm-text">\u0412\u044b \u0438\u0437\u0443\u0447\u0438\u043b\u0438 \u043c\u0430\u0442\u0435\u0440\u0438\u0430\u043b <strong>\u00ab'+step.title+'\u00bb</strong>?</div><button class="btn-mark-done" onclick="confirmStep('+step.idx+')">'+ICO.done+' \u041e\u0442\u043c\u0435\u0442\u0438\u0442\u044c \u0437\u0430\u0432\u0435\u0440\u0448\u0451\u043d\u043d\u044b\u043c</button></div>';}
    } else {
  ...
  /* practice + upload — с разделителем */
  var practiceHtml='';
  ...
  if(finalSteps.length>0){
    var fC=finalSteps.map(function(step){
      ...
      else if(st==='current'){cls='current';sub='<div class="step-subtitle">'+ICO.current+' \u0418\u0442\u043e\u0433\u043e\u0432\u0430\u044f \u0430\u0442\u0442\u0435\u0441\u0442\u0430\u0446\u0438\u044f \u0434\u043e\u0441\u0442\u0443\u043f\u043d\u0430</div>';act='<button class="btn-step open" onclick="openStep('+step.idx+')">'+ICO_OPEN+' \u041d\u0430\u0447\u0430\u0442\u044c</button>';}
